...
class SharedPtr { //Класс, управляеющий определнным ресурсом, считающий ссылки. Сам чистит память при необходимости
 public: //r - ссылка на объект из которого мы будем производить копирование
...
//Дефолтный конструктор.Конструктор, создающий пустой объект(нет ресурса, кот он владеет и нет счетчика ссылок, тк нет русурса)
template <typename T>
SharedPtr<T>::SharedPtr(T* ptr) //Конструктор, кот мы передаем сырой указатель во владение, начинает распоряжаться ресурсом.
    : object(ptr), counter(new std::atomic_uint(1)) { //Инициализируется ресурс этим ptr. Счетчик создается новый и инициализируется 1, тк мы единственные владельцы этого ptr
  if (ptr == nullptr) {
    delete counter;
    counter = nullptr;
  }
...
template <typename T>
SharedPtr<T>::SharedPtr(const SharedPtr& r) //Конструктор копирования.Создает копию объекта из уже существущего.
    : object(r.object), counter(r.counter) { //копируем ресурс и адрес счетчика ссылок
  if (counter != nullptr) (*counter)++; //Увеличивает счетчик ссылок. Тк мы копируем из какого то объекта, то кол-во ссылок должно увеличиться, тк добавился новый объект, управляющий этим ресурсом.
}
template <typename T>
SharedPtr<T>::SharedPtr(SharedPtr&& r) : object(nullptr), counter(nullptr) {
  std::swap(object, r.object);//Меняет местами ресурсы одного метода от одного объекта с ресурсами объекта, кот мы передаем
...
auto SharedPtr<T>::operator=(const SharedPtr& r) -> SharedPtr& {//определяет, что делать с тем ресурсом, кот мы владеем.( уничтожить самому или отдать другим умным указателям)
  if (*this == r) return *this;//проверяем, не присваиваем ли мы себя себе. Тогда просто возвращаем ccылку на самого себя
  reset();
...
auto SharedPtr<T>::operator=(SharedPtr&& r) -> SharedPtr& {// оператор присваивания перемещения
  if (*this == r) return *this; //Сначала решаем, что делать с объектом, кот мы уже владеем, потом перемещаем ресурсы из объекта, который находится справа от знака присваивания.
  reset();
  counter = r.counter;
  object = r.object;
  r.counter = nullptr;
...
template <typename T>
SharedPtr<T>::operator bool() const { //Нулевой ли указатель ил нет
  return object != nullptr;
}
template <typename T>
auto SharedPtr<T>::operator*() const -> T& { //Оператор разыменования
  return *object;
}
template <typename T>
